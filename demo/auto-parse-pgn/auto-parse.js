chess.AutoParse = new Class({
    ...
    checkTimeout: function () {
        ...
        if (this.parsingMode == 0 && elapsed > this.timeout) {
            console.log('stopping engine because of timeout');
    ...
    createController: function () {
        ...
        this.controller = new chess.controller.AnalysisEngineController({
            ...
            listeners: {
                ...
                newGame: that.onNewGame.bind(that),
    ...
    updateMove: function (move) {
       ...
       // console.log(move);
        ...
        if (this.parsingMode == 1 || this.parsingMode == 2) {
            if (!isNaN(score))this.variationUpdate(move, score);
            return;

        }else if(this.parsingMode == 3){
            return;
        }
        if (isNaN(score)) {
            this.controller.stopEngine();
    ...
    onCheckmate: function () {
        ...
        if (checkmateMoves.length > 1) {
            ...
            var fin = this.lastEngineMove.indexOf('#') == -1 && elapsed > this.timeout;
            if (cm > 0 || fin) {
                startTime = new Date().getTime();
                if (fin) {
    ...
    variationTimeout: function () {
        this.parseNextMove();
    },

    variationUpdate: function (move, score) {
        // console.log(score);
        var elapsed = new Date().getTime() - this.startTime;

        var diff = Math.abs(score - this.bestScore);
        if (diff > 3000 || elapsed > 5000) {
            this.parseNextMove();

        } else if (Math.abs(Math.round(score / 20)) >= Math.abs(Math.round(this.bestScore / 20))) {
            if (this.parser.notation != this.currentBestMove) {
                if(this.variationIndex % 2 == 1){
                    // Opponents move
                    this.parsingMode = 3;
                    var fen = this.bestLineFens[this.variationIndex];
                    var m = this.parser.notation;
                    ...
                    this.lineFinder.findLine(fen, m);
                }
            }else{
                this.parseNextMove();
            }
        ...
        // console.log(elapsed, score, this.bestScore);
    },
    ...
    receivedVariationLine:function(variation){
        console.log('received', variation);
        if(variation.length + this.variationIndex == this.bestLines.length){
    ...
    saveGame: function () {
        ...
        this.currentPgn.white = (this.colorToMove == 'white' ? 'White' : 'Black') + ' mates in ' + (Math.ceil(this.currentMoves.length / 2));
